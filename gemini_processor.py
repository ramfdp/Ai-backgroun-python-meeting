...
CHUNK_MS = 15 * 60 * 1000  # ponytail: 15-min chunks
...
def _call_gemini(contents):
    ...
    for attempt in range(3):
        try:
            model = os.getenv("PRIMARY_MODEL", "gemini-2.5-flash") if attempt == 0 else os.getenv("FALLBACK_MODEL", "gemini-3-flash-preview")
            if attempt > 0:
